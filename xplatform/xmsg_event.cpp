...
static std::mutex msg_callback_mutex;

void XMsgEvent::RegCB(xmsg::MsgType type, XMsgEvent::MsgCBFunc func)
{
    std::lock_guard<std::mutex> lock(msg_callback_mutex);
...
{
    //防止Close()后被libevent再次调用
    if (is_closed())
        return;

    // [DBG_PROXY] 标记当前处理的对象
    cout << "[R] this=" << this << " " << server_ip() << ":" << server_port()
         << " " << XGetPortName(server_port()) << endl;
    ...
    while (process_count < MAX_PROCESS)
    {
       ...
       // cout << "#" << flush;
        ...
        // [DBG_PROXY] 消息解析成功
        cout << "[M] msg_type=" << pb_head_->msg_type() << " msg_id=" << pb_head_->msg_id()
             << " msg_size=" << pb_head_->msg_size() << endl;
        ...
        // [DBG_PROXY] 即将调用 ReadCB(head, msg) 虚函数
        cout << "[D] Calling ReadCB(xmsg::XMsgHead*, XMsg*) virtual dispatch" << endl;
        ReadCB(pb_head_, msg);
