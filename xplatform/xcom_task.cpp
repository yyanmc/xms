...
static void SReadCB(struct bufferevent *bev, void *ctx)
{
    ...
    // [DBG_BEV] 追踪每个连接的 bufferevent 读事件
    cout << "[BEV_R] port=" << task->server_port()
         << " svc=" << XGetPortName(task->server_port())
         << " this=" << task
...
int XComTask::Read(void *data, int datasize)
{
    if (!bev_)
    {
        LOGDEBUG("Read: bev not set (closed?)");
        return 0;
    }
...
void XComTask::EventCB(short what)
{
    ...
    if (what & BEV_EVENT_CONNECTED)
    {
        ...
        stringstream ss2;
        ss2 << "connnect server " << server_ip_ << ":" << server_port_ << " " << XGetPortName(server_port_) << " success!";
        LOGINFO(ss2.str().c_str());
        //通知连接成功
        is_connected_ = true;
        is_connecting_ = false;
    ...
    if (what & BEV_EVENT_ERROR )
    {
        ...
        has_error_ = true;
        strncpy(error_, evutil_socket_error_to_string(err), sizeof(error_) - 1);
    ...
    if (what & BEV_EVENT_TIMEOUT)
    {
        ...
        LOGINFO(ss.str().c_str());
        has_error_ = true;
        strncpy(error_, "BEV_EVENT_TIMEOUT", sizeof(error_) - 1);
        error_[sizeof(error_) - 1] = '\0';
        Close();
    }
