...
{
#ifdef _WIN32
    //UTF-8 → UTF-16 → 本地ACP (GBK on Chinese Windows)
    ...
    int clen = WideCharToMultiByte(CP_ACP, 0, wbuf, -1, NULL, 0, NULL, NULL);
...
//本地编码（Windows GBK）转为UTF-8
std::string LocalToUTF8(const std::string &local_str)
{
#ifdef _WIN32
    //本地ACP (GBK) → UTF-16 → UTF-8
    int wlen = MultiByteToWideChar(CP_ACP, 0, local_str.c_str(), -1, NULL, 0);
    if (wlen <= 0) return local_str;
    wchar_t *wbuf = new wchar_t[wlen];
    MultiByteToWideChar(CP_ACP, 0, local_str.c_str(), -1, wbuf, wlen);

    int clen = WideCharToMultiByte(CP_UTF8, 0, wbuf, -1, NULL, 0, NULL, NULL);
    if (clen <= 0) { delete[] wbuf; return local_str; }
    char *cbuf = new char[clen];
...
//UTF-8路径转为系统文件API可用路径
std::filesystem::path U8Path(const std::string &utf8_path)
{
#ifdef _WIN32
    int wlen = MultiByteToWideChar(CP_UTF8, 0, utf8_path.c_str(), -1, NULL, 0);
    if (wlen <= 0) return std::filesystem::path(utf8_path);
    wchar_t *wbuf = new wchar_t[wlen];
    MultiByteToWideChar(CP_UTF8, 0, utf8_path.c_str(), -1, wbuf, wlen);
...
XMutex::XMutex(std::mutex *mux, std::string msg) : XMutex(mux)
{
    msg_ = msg;
}
...
{
    // 辅助函数：格式化为带单位的字符串，保留两位小数
    auto FormatWithUnit = [](long long value, long long divisor, const char* unit) -> std::string {
        long long tmp = (value * 100) / divisor;
        stringstream ss;
        ss << tmp / 100;
        if (tmp % 100 > 0)
            ss << "." << tmp % 100;
    ...
    if (size > 1024LL * 1024 * 1024) //GB
        return FormatWithUnit(size, 1024LL * 1024 * 1024, "GB");
    else if (size > 1024 * 1024) //MB
        return FormatWithUnit(size, 1024 * 1024, "MB");
    else if (size > 1024) //KB
        return FormatWithUnit(size, 1024, "KB");
    else //B
    {
        stringstream ss;
        ss << size << "B";
...
XCOM_API std::string XGetTime(int timestamp, std::string fmt )
{
    ...
    gmtime_r(&tm, &tm_buf);
#endif
    strftime(time_buf, sizeof(time_buf), fmt.c_str(), &tm_buf);
    return time_buf;
}
...
{
#ifdef _WIN32
    //Windows文件API需要本地编码（GBK）
    string local_path = UTF8ToLocal(s);
...
#ifdef _WIN32
    //Windows文件API需要本地编码（GBK）
    string local_path = UTF8ToLocal(path);
...
XCOM_API std::list< XToolFileInfo > GetDirList(std::string path)
{
    ...
    do
    {
        ...
        file_info.filename = LocalToUTF8(file.name);
...
%% 百分号下面的程序则显示当前的完整日期：
        ...
        */
        time_t tm = file_info.time_write;
        {
            ...
            gmtime_r(&tm, &gm_tm);
#endif
            strftime(time_buf, sizeof(time_buf), "%F %R", &gm_tm);
        }
...
XCOM_API std::string GetDirData(std::string path)
{
    ...
    do
    {
        ...
        char buf[1024] = { 0 };
        snprintf(buf, sizeof(buf), "%s,%u;", file.name, file.size);
        data += buf;
    } while (_findnext(dir, &file) == 0);
    _findclose(dir);
#else
    ...
    while((entry = readdir(dp))!=NULL)
    {
	    ...
	    snprintf(buf, sizeof(buf), "%s,%llu;", entry->d_name, (unsigned long long)statbuf.st_size);
	    data += buf;
    }
    closedir(dp);
#endif
...
#ifdef _WIN32
    //Windows文件API需要本地编码（GBK）
    string dir_new = UTF8ToLocal(path);
#else
    string dir_new = path;
#endif
    string name = "";
...
std::string XGetHostByName(std::string host_name)
{
    ...
    {
        stringstream ss;
        ss << "XGetHostByName failed! cannot resolve " << host_name;
...
class CXAES :public XAES
{
    ...
    virtual bool SetKey(const char *key, int key_size, bool is_enc) override
    {
        ...
        {
            stringstream ss;
            ss << "AES key size error(>32 && <=0 )! key_size= " << key_size;
            LOGERROR(ss.str().c_str());
            return false;
        }
