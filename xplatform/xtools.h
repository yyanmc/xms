...
//    XMS_LOG_LEVEL_ERROR
//};
//#define CUR_LEVEL  XMS_LOG_LEVEL_INFO
//
...
//将UTF-8字符串转换为当前控制台编码（Windows下UTF-8→ACP, Linux下直接返回）
XCOM_API std::string UTF8ToLocal(const std::string &utf8_str);
//将本地编码（Windows GBK）转为UTF-8
XCOM_API std::string LocalToUTF8(const std::string &local_str);
...
//将UTF-8路径转为系统文件API可用的路径（Windows将UTF-8→UTF-16，其他系统直接返回）
XCOM_API std::filesystem::path U8Path(const std::string &utf8_path);